...
int getSemSetId(const std::string &pathname, int projId, int maxSlots) {
  ...
  if (semId != -1) {
    ...
    // Initialize the # of empty slots to maxSlots
    arg.val = maxSlots;
    if (semctl(semId, IDX_EMPTY, SETVAL, arg) == -1) {
      errExit("semctl(semId, 1, SETVAL, " + std::to_string(maxSlots) + ")");
    }
